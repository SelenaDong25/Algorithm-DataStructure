...
This program is written by using instructor provided base code and 
added additional block code.
 */
package searching;
...
public class SearchExamples {
    ...
    /**
     * This method use analogous code as "useLinerSearch" and modified corresponding line.
     * @param sampleSize define number of times to run the search on each dataset size
     * @param dataSetSize represent array size
     * @return the count of data comparison times
     */
    private static long usingBinarySearch(int sampleSize, int dataSetSize) {
        ...
        for (int i = 0; i < sampleSize; i++) {
            int needle = rand.nextInt(haystack.length);   // random value 
            LwtSearch.binarySearch(haystack, needle);
            compareCt += LwtSearch.compareCt;
        }
        return compareCt / sampleSize;
    }
